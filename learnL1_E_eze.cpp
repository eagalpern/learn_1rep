...
std::uniform_int_distribution<int> uni32(0,32); // guaranteed unbiased  *check*
...
int main()
{
	...
	const int npos=33; // length sequences *check* 
	...
	while(error > maxErrorPermited){
		...
		while(seqsavecounter<TotalIterations)
		{
			...
			int mpos= uni32(rng); // sospecho que aca hay que cambiar por 32
			int mres= uni20(rng); 
