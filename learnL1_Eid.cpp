...
int main()
{
	...
	const int npos=66; // length sequences *check*
	const int npid=34;    // number of ID Lagrange Multipliers *check* (npos/2+1)
	...
	const double epsilonID=0.05;       // epsilon to renew parameters lambda identity
	...
	ifstream f1File("frecuenciasmarginales_train.txt");  /*check*/
	...
	ifstream f2File("frecuenciasconjuntas_train.txt");  /*check*/
	...
	while(error > maxErrorPermited){
		...
		{
			m=1.5*abs(pid_dis[i]/TotalSequences - pid_input[i]);
			if(m>maxPij){maxPij=m;}
		}
		error=maxPij;
