...
// Hamiltoniano:  sum h + sum sum J + sum lambda_id: Agrego multiplicadores de Lagrange de pId
//g++ -std=gnu++11 -o learnL1_Eid learnL1_Eid.cpp
...
std::uniform_int_distribution<int> uni65(0,65); // guaranteed unbiased  *check*
...
int main()
{
	...
	const int npos=48; // length sequences *check*
	const int npid=25;    // number of ID Lagrange Multipliers *check* (npos/2+1)
	const int npos2=npos/2; // length 1 repeat
	...
	const double epsilonJoint=0.05;   // epsilon to renew parameters jij
	const double gamma=0.001;
	const double epsilonID=5;       // epsilon to renew parameters lambda identity
	const double maxErrorPermitedFJ=0.015;  // maximum error allowed between pij-fij and pi-fi
	const double maxErrorPermitedFM=0.01;  // maximum error allowed between pij-fij and pi-fi
	const double maxErrorPermitedID=0.002;  // maximum error allowed between pij-fij and pi-fi
	...
	ifstream f1File("frecuenciasmarginales_CDHIT.txt");  /*check*/
	...
	ifstream f2File("frecuenciasconjuntas_CDHIT.txt");  /*check*/
	...
	ifstream pidFile("pid_data.txt");		/*check*/
	...
	ifstream j0File("Final_jij_v1.txt"); /*check*/
	...
	ifstream h0File("Final_h_v1.txt"); /*check*/
	for(int i=0;i<naanpos;i++){
		h0File >> hi[i];
	}
	h0File.close();
	...
	while(error){
		...
		if((maxPij<maxErrorPermitedFJ) && (maxPi<maxErrorPermitedFM) && (maxID<maxErrorPermitedID)){error=false;}
		...
		if(writeFiles){
			...
			errorFile << maxPi << "\t" << maxPij << "\t" <<  maxID << endl;
